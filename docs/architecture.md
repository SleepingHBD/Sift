...
The Evidence Inbox is an application projection over those normalized references. The `search_evidence_page` database function unions only records already authorized by project Row Level Security, returns the shared evidence-reference shape without copying source content, and omits unassigned Personal Radar records from the project-level queue. Search uses PostgreSQL full-text indexes for preserved source text and also covers strategist notes, authors, topics, tags, and provenance. The detail drawer keeps source evidence visibly separate from interpretation.
...
Relationship inspection is a single RLS-scoped `list_evidence_relationships` call made only for the evidence currently open in the detail drawer or deletion dialog. It returns tags, project links, saved destinations, private assets, notes, trends, and strategic citations without copying the source record. Reverse lookup indexes cover `insight_sources` and `brief_sources`, avoiding a browser-side N+1 scan.
