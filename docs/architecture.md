...
- Strategy work reuses project-scoped `strategy_sessions` and `strategy_stages`. `strategy_session_turns` adds the append-only working conversation: browser clients may write only their own strategist turns inside an accessible project and cannot forge assistant or ChatGPT provenance. The authenticated Strategy AI function can idempotently attach a server-validated manual ChatGPT response as an assistant turn. `strategy_session_pieces` preserves its observations, questions, interpretations, tensions, hypotheses, and opportunities as optional working memory; `strategy_session_piece_sources` keeps original citations inspectable, and browser users may change only a piece's dismiss/restore status. `strategy_session_inputs` records Signal or saved Strategy AI provenance without treating derived analysis as evidence; `strategy_stage_sources` cites same-project mention, Research, or Inspiration records as support, contradiction, or context. Same-session dependencies reject cycles, alternatives remain retained records, material edits demote approved stages, and database triggers write append-only stage and alternative revisions. Authenticated clients cannot delete conversation turns, generated pieces, selected stages, alternatives, or revisions directly. Strategy Sessions is the default working interface; the formal stage pipeline remains available as Review argument.
