...
- Strategy work reuses project-scoped `strategy_sessions` and `strategy_stages`. `strategy_session_turns` adds the append-only working conversation: browser clients may write only their own strategist turns inside an accessible project and cannot forge assistant or ChatGPT provenance. `strategy_session_inputs` records Signal or saved Strategy AI provenance without treating derived analysis as evidence; `strategy_stage_sources` cites same-project mention, Research, or Inspiration records as support, contradiction, or context. Same-session dependencies reject cycles, alternatives remain retained records, material edits demote approved stages, and database triggers write append-only stage and alternative revisions. Authenticated clients cannot delete conversation turns, selected stages, alternatives, or revisions directly. Strategy Sessions is the default working interface; the formal stage pipeline remains available as Review argument.
...
| `/strategy-ai` | What evidence-backed interpretation can I develop through the accepted manual ChatGPT handoff? This remains a compatibility route during the conversational transition. |
