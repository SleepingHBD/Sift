...
- Strategy work reuses project-scoped `strategy_sessions` and `strategy_stages`. `strategy_session_inputs` records Signal or saved Strategy AI provenance without treating derived analysis as evidence; `strategy_stage_sources` cites same-project mention, Research, or Inspiration records as support, contradiction, or context. Same-session dependencies reject cycles, alternatives remain retained records, material edits demote approved stages, and database triggers write append-only stage and alternative revisions. Authenticated clients cannot delete selected stages, alternatives, or revisions directly.
