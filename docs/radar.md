...
The interface asks the authenticated `radar_scheduler_status` RPC whether the cron job and required Vault records are present. Daily or weekly choices remain off and are labelled unavailable when that check fails. Cron activation is deliberately separate from schema deployment because it authorizes unattended external requests.

Retention remains non-destructive. A monitor can keep raw conversations forever or store a future 90-day, 180-day, or 365-day cleanup preference. The authenticated `radar_retention_preview` function counts records older than the selected cutoff and separates conversations protected by saved links, strategic citations, notes, important markers, or reviewed evidence. It never deletes data. Actual cleanup requires a separate, audited retention worker and confirmation policy in a later increment.
