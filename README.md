...
Live generation is deliberately unconfigured. Activating it later requires `OPENAI_API_KEY`, an explicit `OPENAI_STRATEGY_MODEL`, and server-side monthly request and token limits in Edge Function secrets; none belongs in a `NEXT_PUBLIC_` variable, GitHub Pages bundle, database row, or repository file. Until every guardrail is present, the interface clearly labels generation as unavailable and makes no OpenAI request. A five-scenario evaluation suite is ready for the first controlled live test. General brainstorming remains a separate future mode rather than being presented as workspace evidence. See [docs/strategy-ai.md](docs/strategy-ai.md) and [docs/strategy-ai-evaluation.md](docs/strategy-ai-evaluation.md).
