...
- One cloud-backed Library for Research, Radar sources, notes, files, social captures, and Inspiration, with clear source filters, lightweight inspiration capture, project assignment, search, source links, protected deletion, CSV evidence import, and reviewed browser-data migration
...
Phase 7 is now in progress under the notebook-first direction. A strategist can begin with a named notebook page, write or capture gradually, attach original Radar and Library evidence, connect entries, inspect emerging threads, and open the formal argument only when structure becomes useful. Radar conversations and Library sources can be sent directly to a page with one optional thought; Sift prevents accidental duplicate placement on that page and exposes the notebook citation in the source relationship trail. The former Trends workspace now lives inside Radar as **Signals**, and the former Inspiration workspace now lives inside the **Library** as a source filter with a lightweight save flow. Both legacy routes remain compatible. The existing Observation → Pattern → Tension → Insight → Opportunity → Strategic Proposition review remains available with confidence, gaps, alternatives, dependencies, approval checks, and revision history. No deterministic or imported AI suggestion becomes accepted strategy automatically. See [docs/phase-7-audit.md](docs/phase-7-audit.md) and [docs/development-roadmap.md](docs/development-roadmap.md).
