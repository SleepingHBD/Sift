...
Phase 7 is now in progress. Its schema audit, trusted-pipeline foundation, project-scoped Insight Builder, uncertainty/traceability layer, and Strategic Proposition stage are implemented. Under **Think → Insight Builder**, a strategist can create durable project sessions; edit Observation → Pattern → Tension → Insight → Opportunity → Strategic Proposition; link supporting, challenging, and contextual evidence; inspect original sources; and retain Signals or saved Strategy AI responses as clearly separated provenance. Each saved stage supports explicit confidence, research gaps, competing interpretations, earlier-stage dependencies, Draft/Ready/Approved states, database-enforced approval checks, and append-only revision history. The proposition unlocks only after a saved Opportunity, automatically retains its direct Opportunity link, and displays the complete inherited stage and original-source trail without copying evidence. Insight Builder citations protect their original sources from deletion and Radar retention until the citation is deliberately removed. See [docs/phase-7-audit.md](docs/phase-7-audit.md).
