  ...
  return (
    <div className="page page--home personal-home">
      ...
      <section className="strategist-input-section strategist-input-section--blank">
        ...
        {answerVisible ? <Card className="quick-answer general-ai-notice"><div className="quick-answer__icon"><Sparkles size={18} /></div><div><div className="quick-answer__label"><strong>{hasEvidence ? "Workspace-backed workflow" : "Evidence needed"}</strong><span>{hasEvidence ? "Think gradually with your saved evidence" : "No finding generated"}</span></div><p>{hasEvidence ? "Open a Strategy Session to talk through the question one step at a time. The verified ChatGPT handoff remains available whenever you want outside assistance." : "I don’t have enough workspace evidence yet to answer this as a research-backed strategist. Add evidence and conversations first so future analysis can cite your sources."}</p><div className="general-ai-actions">{hasEvidence ? <Link href="/insight-builder">Open Strategy Sessions <ArrowRight size={13} /></Link> : <><Link href="/evidence">Open Evidence <ArrowRight size={13} /></Link><Link href="/radar/#new-monitor">Create Radar monitor <ArrowRight size={13} /></Link></>}</div></div></Card> : null}
      </section>
