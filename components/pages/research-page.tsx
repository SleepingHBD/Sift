...
import { BookOpen, Cloud, ExternalLink, FileImage, FileSpreadsheet, FileText, LoaderCircle, MessageSquareText, Plus, Search, Trash2, Upload, X } from "lucide-react";
  ...
  return (
    <div className="page">
      <PageIntro eyebrow="Research library" title="Evidence, with a point of view." description="Collect source material, annotate what matters, and connect every strategic leap back to proof.">
        {projects.length ? <><Button onClick={() => setCsvImportOpen(true)}><FileSpreadsheet size={16} />Import CSV</Button><Button onClick={() => openCaptureDialog("file")}><Upload size={16} />Upload file</Button><Button variant="dark" onClick={() => openAdd("Note")}><Plus size={16} />Add research</Button></> : <Button variant="dark" onClick={() => setProjectDialogOpen(true)}><Plus size={16} />Create project first</Button>}
      </PageIntro>
      ...
      {workspaceStatus === "loading" && !researchItems.length ? (
        ...
        <EmptyState icon={BookOpen} title="Start building your knowledge base." description={projects.length ? "Add a source, import a research spreadsheet, capture a social post, or upload a private file. Every item stays attached to its evidence trail." : "Create a project first, then add articles, URLs, social posts, reports, statistics, quotes, notes, screenshots, and documents as evidence."} actions={projects.length ? <><Button variant="dark" onClick={() => openAdd("Article")}><FileText size={15} />Add research</Button><Button onClick={() => setCsvImportOpen(true)}><FileSpreadsheet size={15} />Import CSV</Button><Button onClick={() => openCaptureDialog("social")}><MessageSquareText size={15} />Capture social</Button><Button onClick={() => openCaptureDialog("file")}><Upload size={15} />Upload file</Button></> : <Button variant="dark" onClick={() => setProjectDialogOpen(true)}><Plus size={15} />Create your first project</Button>} />
      ) : (
        <>
      ...
      {csvImportOpen ? <EvidenceCsvImportDialog projects={projects} initialProjectId={projectFilter === "all" ? activeProjectId : projectFilter} onClose={() => setCsvImportOpen(false)} onImported={() => retryWorkspace()} /> : null}
    </div>
  );
}
