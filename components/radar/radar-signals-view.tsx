...
import { CheckCircle2, CircleDashed, Eye, Lightbulb, LoaderCircle, Plus, Radar, RotateCcw, Search, XCircle } from "lucide-react";
...
export function RadarSignalsView() {
  ...
  return (
    <section className="radar-signals-view" aria-labelledby="radar-signals-title">
      <header className="radar-signals-view__intro">
        <div><p className="eyebrow">Signals</p><h2 id="radar-signals-title">Notice change before naming a trend.</h2><p>Keep early observations here while you collect support, context, and contradictions. Promote one only when its evidence can carry the claim.</p><small>Candidate → Watching → Promoted trend</small></div>
        {projects.length ? <Button variant="dark" onClick={() => setDialogOpen(true)}><Plus size={15} />New candidate</Button> : <Button variant="dark" onClick={() => setProjectDialogOpen(true)}><Plus size={15} />Create project</Button>}
      </header>
      ...
      {workspaceStatus === "loading" && !projects.length ? (
        ...
        <>
          ...
          {loading ? (
            ...
            <EmptyState icon={CircleDashed} title={projectFilter === "all" ? "No signals recorded yet." : "No signals in this project yet."} description="Record a specific observation or hypothesis worth checking. It remains a working signal until evidence supports more." actions={<><Button variant="dark" onClick={() => setDialogOpen(true)}><Plus size={15} />Record a candidate</Button><Link className="ui-button ui-button--secondary ui-button--md" href="/evidence">Review Library</Link></>} />
          ) : (
