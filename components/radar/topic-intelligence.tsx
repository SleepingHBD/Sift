...
export function TopicIntelligence({ topics, mentions, activeTopic, supportingStatus, supportingError, onSelect, onOpenMention, onInspectMentions }: { topics: TopicIntelligenceType[]; mentions: RadarMention[]; activeTopic: string; supportingStatus?: "idle" | "loading" | "error"; supportingError?: string; onSelect: (topic: string) => void; onOpenMention: (mention: RadarMention) => void; onInspectMentions: () => void }) {
  ...
  const missingExamples = selected ? Math.max(0, selected.exampleMentionIds.length - examples.length) : 0;
  return (
