...
export function SpikeDrawer({ spike, mentions, onClose, onOpenMention }: { spike: SpikeInsight | null; mentions: RadarMention[]; onClose: () => void; onOpenMention: (mention: RadarMention) => void }) {
  ...
  return (
    <div className="radar-overlay radar-overlay--drawer" role="dialog" aria-modal="true" aria-labelledby="spike-drawer-title">
      ...
      <aside className="radar-drawer spike-drawer">
        ...
        <section className="spike-top-mentions"><span className="drawer-section-label">Highest-engagement mentions</span>{topMentions.map((mention) => <button key={mention.id} onClick={() => onOpenMention(mention)}><div><strong>{mention.author}</strong></div><p>{mention.content}</p><span>{mention.topics[0]} · {formatNumber(mention.engagement)} engagement <ArrowUpRight size={12} /></span></button>)}{missingMentions ? <p>{missingMentions} supporting record{missingMentions === 1 ? " is" : "s are"} outside the conversation history currently loaded in this browser.</p> : null}</section>
